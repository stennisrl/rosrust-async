...
//!
//! This example is designed to run in tandem with `publisher`, however
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("subscriber=info".parse()?))
        .init();

    let node = NodeBuilder::new().name("/subscriber").build().await?;

    info!("Subscribing to /chatter, press Ctrl+C to exit.");

    let mut subscriber = node.subscribe::<RosString>("/chatter", 1, false).await?;

    loop {
        tokio::select! {
            msg = subscriber.recv() => {
                match msg{
                    Ok(msg) => info!("Got a message: {msg:?}"),
                    Err(e) => {
